...
export const plansService = {
   ...
   */
  async getPlans() {
    const response = await adminApiClient.get(endpoints.plans.list)
  ...
  async getPlan(id) {
    try {
      const response = await adminApiClient.get(endpoints.plans.get(id))
      const data = response.data?.data
      if (data) return data
    } catch (err) {
  ...
  async createPlan(body) {
    const response = await adminApiClient.post(endpoints.plans.create, body)
    const data = response.data?.data
    if (!data) {
  ...
  async updatePlan(id, body) {
    const response = await adminApiClient.put(endpoints.plans.update(id), body)
    const data = response.data?.data
    if (!data) {
   ...
   */
  async deletePlan(id) {
    const response = await adminApiClient.delete(endpoints.plans.delete(id))
    const data = response.data?.data
