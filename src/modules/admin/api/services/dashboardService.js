...
const dashboardService = {
  ...
  async getSummary() {
    const data = unwrap(await adminApiClient.get(endpoints.dashboard.summary))
    return {
      counts: data.counts ?? {},
  ...
  async getPendingKyc(limit = 10) {
    const data = unwrap(
      await adminApiClient.get(endpoints.dashboard.pendingKyc, {
  ...
  async getCommissionStats(periodKey, usePrevious = false) {
    ...
    const data = unwrap(
      await adminApiClient.get(endpoints.dashboard.commissionStats, { params })
    )
  ...
  async getInvestmentVolume(periodKey) {
    ...
    const data = unwrap(
      await adminApiClient.get(endpoints.dashboard.investmentVolume, { params })
    )
  ...
  async getUserGrowth(periodKey) {
    const params = toParams(periodKey)
    const data = unwrap(
      await adminApiClient.get(endpoints.dashboard.userGrowth, { params })
    )
    const series = (data.series ?? []).map((row) => ({
  ...
  async getInvestmentByPlan() {
    const data = unwrap(await adminApiClient.get(endpoints.dashboard.investmentByPlan))
  ...
  async getInstallmentSummary() {
    const data = unwrap(await adminApiClient.get(endpoints.dashboard.installmentSummary))
    return {
  ...
  async getPendingPaymentVerifications() {
    const data = unwrap(
      await adminApiClient.get(endpoints.purchases.pendingVerification)
    )
  ...
  async getRecentPurchases(limit = 10) {
    const data = unwrap(
      await adminApiClient.get(endpoints.purchases.list, {
