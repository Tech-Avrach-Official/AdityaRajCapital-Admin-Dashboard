...
export const kycService = {
  ...
  async getKYCList(params = {}) {
    ...
    try {
      ...
      const res = await adminApiClient.get(endpoints.kyc.list, { params: apiParams })
      ...
      if (status === "pending" || !status) {
        try {
          const dashRes = await adminApiClient.get(endpoints.dashboard.pendingKyc, {
            params: { limit },
          })
  ...
  async getInvestorKycDocuments(investorId) {
    const data = unwrap(
      await adminApiClient.get(endpoints.admin.investorKycDocuments(investorId))
    )
    return {
  ...
  async getPartnerKycDocuments(partnerId) {
    const data = unwrap(
      await adminApiClient.get(endpoints.admin.partnerKycDocuments(partnerId))
    )
    return {
